...
export class PatientDetailsComponent implements OnInit {
    ...
    if (this.patientId) {
      ...
      this.patientService.getPatientData$(this.patientId).subscribe(
        response => {
          ...
          this.setFormValues(this.patient.name, this.patient.lastName, this.patient.age, this.patient.phone, this.patient.medicines, this.patient.allergies, this.patient.moles, this.patient.bodyMetals, this.patient.tattoos, this.patient.skinCancer, this.patient.epilepsy, this.patient.pregnant, this.patient.previousTreatment );
          this.spinner.hide();
        },
        error => {
          this.spinner.hide();
  ...
  private setFormValues(name = null, lastName = null, age = null, phone = null, medicines = null, allergies = null,
    moles = null, bodyMetals = null, tattoos = null, skinCancer = false, epilepsy = false, pregnant = false, previousTreatment = null): void {
    this.patientForm = this.fb.group({
      ...
      patientMedicines: new FormControl(
        { value: medicines, disabled: !this.editionMode }
      ),
      patientAllergies: new FormControl(
        { value: allergies, disabled: !this.editionMode }
      ),
      patientMoles: new FormControl(
        { value: moles, disabled: !this.editionMode }
      ),
      patientBodyMetals: new FormControl(
        { value: bodyMetals, disabled: !this.editionMode }
      ),
      patientTattoos: new FormControl(
        { value: tattoos, disabled: !this.editionMode }
      ),
      patientSkinCancer: new FormControl(
        { value: skinCancer, disabled: !this.editionMode }
      ),
      patientEpilepsy: new FormControl(
        { value: epilepsy, disabled: !this.editionMode }
      ),
      patientPregnant: new FormControl(
        { value: pregnant, disabled: !this.editionMode }
      ),
      patientPreviousTreatment: new FormControl(
        { value: previousTreatment, disabled: !this.editionMode }
      ),
    });
  ...
  private onChangesForm(): void {
    ...
    this.patientForm.get('patientPreviousTreatment').valueChanges.subscribe(val => {
